...
<div x-data="{ openModal: @entangle('showSaveModal') }">
    {{-- ── Page Header ───────────────────────────────────────────────────────── --}}
    <div class="mb-6">
        <h1 class="text-heading-lg text-ink">Tambah Kategori</h1>
        <p class="text-body-md text-mute">Buat kategori produk baru</p>
    </div>
        ...
        <form wire:submit.prevent="confirmSave" class="space-y-6">
            ...
            <div>
                <label for="input-name" class="block text-[14px] font-semibold text-ink mb-1.5">
                    Nama Kategori <span class="text-primary">*</span>
                </label>
                <input wire:model="name" id="input-name" type="text" placeholder="Contoh: Laptop, Monitor..." autofocus
            ...
            {{-- Description --}}
            <div>
                <label for="input-description" class="block text-[14px] font-semibold text-ink mb-1.5">
                    Deskripsi
                </label>
                <textarea wire:model="description" id="input-description" rows="4" placeholder="Deskripsi singkat kategori (opsional)..."
                ...
                @error('description') <p class="mt-1.5 text-xs font-semibold text-primary">{{ $message }}</p> @enderror
            </div>
            ...
            <div class="flex items-center justify-end gap-3 pt-4 border-t border-hairline">
                <a href="{{ route('categories.index') }}" wire:navigate class="btn-secondary">
                    Batal
                </a>
    ...
    <div x-show="openModal"
        ...
        <div x-show="openModal"
            ...
            <h3 class="text-heading-lg text-ink mb-2">Simpan Kategori?</h3>
            <p class="text-body-md text-body mb-8">
                Apakah Anda yakin ingin menyimpan kategori baru ini ke database?
            </p>
